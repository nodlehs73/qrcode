...
# Am un fisier de input cu mai multe fisiere (poze) si pentru fiecare apelez functia de citire
# Din bitii [ 4 : 7 ] aflu al catelea cod este si il pun in matricea de raspuns final la pozitia respectiva
# La final asamblez mesajul din matricea de raspuns final

def read_qr_code_structured_append():
    answear_dictionary = {}
    ...
    for file_path in os.listdir(folder_path):
        ...
        binary_matrix, version  = png_to_binary_matrix(folder_path + '/' + file_path) # Versiunea o primim ca parametru
        n = 17 + version * 4
        ...
        mask_pattern = int(EC_level_and_mask_list[4]) + int(EC_level_and_mask_list[3]) * 2 + int(
            EC_level_and_mask_list[2]) * 4

        # Am aflat mask pattern, deci acum aducem matricea la forma initiala, inainte de aplicarea mastii
        # Modific binary_matrix
        ...
        # Stim sirul secvential in binar, pornim de dupa character count si sarim din 8 in 8, pana aflam toate caracterele din mesaj

        mode_indicator_and_character_count = 20 + 4 + character_count_size

        number_of_characters_in_message = int(sequential_bit_string_of_codewords[24: (mode_indicator_and_character_count)],
                                              2)
        ...
        # Aflam indexul codului qr curent, adica al catelea din secventa de coduri qr este - la bitii [4 : 7], nr de coduri din secventa si paritatea
        position_in_final_answear_matrix = int(sequential_bit_string_of_codewords[4: 8], 2)
        number_of_qr_codes_in_message = int(sequential_bit_string_of_codewords[8: 12], 2)
        message_parity = int(sequential_bit_string_of_codewords[12: 20], 2)

        if message_parity not in answear_dictionary:
            answear_dictionary[message_parity] = ["" for i in range(number_of_qr_codes_in_message + 1)]
        ...
        if character_count_size == 8:
            number_of_characters_in_total_sequential_bit_string_of_rsc_codewords = int(
                total_sequential_bit_string_of_rsc_codewords[24:32], 2)
        else:
            number_of_characters_in_total_sequential_bit_string_of_rsc_codewords = int(
                total_sequential_bit_string_of_rsc_codewords[24:40], 2)
        ...
        for i in range(number_of_characters_in_total_sequential_bit_string_of_rsc_codewords):
            if character_count_size == 8:
                character_at_position_i_in_total_sequential_bit_string_of_rsc_codewords = chr(
                    int(total_sequential_bit_string_of_rsc_codewords[32 + i * 8: 32 + (i + 1) * 8], 2))
                encoded_message += character_at_position_i_in_total_sequential_bit_string_of_rsc_codewords
            else:
                character_at_position_i_in_total_sequential_bit_string_of_rsc_codewords = chr(
                    int(total_sequential_bit_string_of_rsc_codewords[40 + i * 8: 40 + (i + 1) * 8], 2))
                encoded_message += character_at_position_i_in_total_sequential_bit_string_of_rsc_codewords


        answear_dictionary[message_parity][position_in_final_answear_matrix] = encoded_message

    print (f"In folder avem {len (answear_dictionary)} mesaje encodate")
    for parity in answear_dictionary:
        complete_message = ""

        for i in range(len(answear_dictionary[parity])):
            complete_message += answear_dictionary[parity][i]

        print(f"Mesajul complet este {complete_message}")
